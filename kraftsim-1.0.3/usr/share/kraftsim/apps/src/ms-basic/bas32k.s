...
WRKSPC  .EQU    0x4000               ; BASIC Work space
...
MLOOP:  INC     HL              ; Next byte
	...
	;================================================
        ld      a,h             ; Above address F400 ? - Kraft 80
        cp      #0xF4
        jr      z,SETTOP
...
TSTMEM: CALL    ATOH            ; Get high memory into DE
	...
	;================================================
        ld      a,h		; Limits top at 0xF400 for Kraft 80
        cp	#0xF4
        jr      c,tstmem_1a
        ld      hl,#0xF400
tstmem_1a:
...
ERRMSG: .ascii  " Error\0"
      ;  .byte   0
INMSG:  .ascii  " in "
ZERBYT: .byte   0               ; A zero byte
OKMSG:  .ascii  "Ok\r\n\0\0"
      ;  .byte   CR,LF,0,0
BRKMSG: .ascii  "Break\0"
      ;  .byte   0
