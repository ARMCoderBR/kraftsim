...
int getBootSector (void){
    ...
    uint16_t dirEntriesperSector = (fatdata.bytesPerSector >> 5);
    uint16_t rootDirSectors = fatdata.rootDirEntries / dirEntriesperSector;

    if (fatdata.rootDirEntries % dirEntriesperSector)
        rootDirSectors++;

    fatdata.rootDirSectors = rootDirSectors;

    uint16_t logicalSectorsPerFat = fatdata.logicalSectorsPerFat;
    fatdata.rootDirStartSector = (uint32_t)fatdata.reservedSectors;
    while (logicalSectorsPerFat--)
        fatdata.rootDirStartSector += (uint32_t)fatdata.numberOfFATs;//*(uint32_t)fatdata.logicalSectorsPerFat;
...
uint32_t getSectorFromCluster (uint16_t cluster){
    ...
    uint16_t sectorsPerCluster = fatdata.sectorsPerCluster;

    uint32_t res = fatdata.ssaIndex;

    while(sectorsPerCluster--)
        res += ((uint32_t)cluster-2);
...
int ch376_listdir(void){
    ...
    int res = 1;
    uint32_t sec = fatdata.partitionLogicalSector + fatdata.rootDirStartSector;
...
void ch376_scan_fat(void){
    ...
    uint16_t nBlocks = (fatdata.totalLogicalSectors - fatdata.ssaIndex) / fatdata.sectorsPerCluster;
    ...
    uint16_t kbytesPerCluster = fatdata.bytesPerSector*fatdata.sectorsPerCluster/1024;

    printdec(freeBlocks*kbytesPerCluster);
    putstr(" kBytes free\r\n");
    putcrlf();
}
