...
STACKTOP:
isr0vector:	.ds	2		;0xF800
isr1vector:	.ds	2		;0xF802
isr2vector:	.ds	2		;0xF804
isr3vector:	.ds	2		;0xF806
isr4vector:	.ds	2		;0xF808
isr5vector:	.ds	2		;0xF80A
isr6vector:	.ds	2		;0xF80C
isr7vector:	.ds	2		;0xF80E
diskvector:	.ds	2		;0xF810
timecount:	.ds	1		;0xF812

CMD_BUFSZ	.equ	96
cmd_buf:	.ds	CMD_BUFSZ	;0xF813
...
timer_isr:	ld	a,(timecount)
		inc	a
		ld	(timecount),a
		jp	ch376_timer_cb

ps2_isr:	ld	a,(bufkeyqty)
		cp	#BUFKEYSIZE
		jr	nz,key_isr1
		in	a,(PORTKEY)
		ret
...
tx_char:	out	(PORTSERDATA),a

wait_tx:	in	a,(PORTSERSTATUS)
		bit	1,a
		jr	nz,wait_tx
		ret
...
has_rxchar:	ld	a,(bufrxqty)
		or	a
		ret

		;///////////////////////////////////////////////////////////////

rx_char:	ld	a,(bufrxqty)
		or	a
		jr	nz,rx_char0
...
putclr:		ld	a,#0x10		;Reset scroller
		out	(PORTMODE),a
		ld	a,#0x20
		out	(PORTMODE),a
...
scroll_crt0b:	ld	(lastlineptr),hl
		ld	a,l
		out	(PORTADDRL),a
		ld	a,h
		out	(PORTADDRH),a
...
keyt6a:		cp	#0x59			; RShift	
		jr	nz,keyt6b
		res	1,(hl)
		jr	keyt6c

keyt6b:		cp	#0x14			; LCTRL	
		jr	nz,keyt6c
		res	4,(hl)
...
keyt3a:		cp	#0x12			; LShift	
		jr	nz,keyt3b
		set	0,(hl)
		jr	keyt4

keyt3b:		cp	#0x59			; RShift	
		jr	nz,keyt3c
		set	1,(hl)
		jr	keyt4
...
keyt3d:		cp 	#0x14			; LCTRL
		jr	nz,keyt3e
		set	4,(HL)
		jr	keyt4

keyt3e:		ld	hl,#tab_xlat
		ld	c,a
...
keyt4a:		or	a			; not 0 when there's a valid symbol
...
tab_xlat:	; US QWERTY compatible symbols
